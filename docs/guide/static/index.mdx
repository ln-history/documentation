...
# Static Data Insertion <Badge text="easy" type="info" />
...
Create an `.env` file and fill in the **url of your blockchain rpc explorer** as well as the file_path to the file with the raw gossip.
The easiest way would be to just copy your `gossip_store` file in this directory.

```bash
FILE_PATH=gossip_store                                                  # Or any other file path
...
Please make sure that the permissions are correctly set, such that the insert-gossip service has the correct permission to read the file at `FILE_PATH`. <Badge text="info" type="tip" />
