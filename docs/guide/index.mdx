...
We will follow the [official documentation](https://docs.docker.com/engine/install/ubuntu/).

1. Setup up Docker's `apt` repository
<div className="terminal">
    <div className="terminal-content">
        {String.raw`# Add Docker's official GPG key:`}
        <TerminalOneLiner command="sudo apt-get update" inTerminal={true} />
        <TerminalOneLiner command="sudo apt-get install ca-certificates curl" inTerminal={true} />
        <TerminalOneLiner command="sudo install -m 0755 -d /etc/apt/keyrings" inTerminal={true} />
        <TerminalOneLiner command="sudo curl -fsSL https://download.docker.com/linux/ubuntu/gpg -o /etc/apt/keyrings/docker.asc" inTerminal={true} />
        <TerminalOneLiner command="sudo chmod a+r /etc/apt/keyrings/docker.asc" inTerminal={true} />

        {String.raw`# Add the repository to Apt sources:`}
        <TerminalOneLiner command={String.raw`echo \"deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu \$(. /etc/os-release && echo "\${UBUNTU_CODENAME:-$VERSION_CODENAME}") stable" | \sudo tee /etc/apt/sources.list.d/docker.list > /dev/null sudo apt-get update`} inTerminal={true} />
    </div>
</div>


2. Install the (latest) Docker packages
<TerminalOneLiner command="sudo apt-get install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin" inTerminal={false} />
