...
<Title text='Database - Creation' badgeType='tip' badgeText='easy' />
...
erDiagram
    ...
    nodes_raw_gossip {
        ...
        varchar(66) node_id FK
        tztimestamp timestamp
        bytea raw_gossip
    }

    channels {
        varchar(64) gossip_id 
        varchar(23) scid PK
        varchar(66) source_node_id FK
        varchar(66) target_node_id FK
        tztimestamp from_timestamp
...
<div className="docker-terminal">
    <DockerTerminalOneLiner pathInfo="admin=#"command={String.raw`-- Create the ln-history-database
    CREATE DATABASE "ln-history-database" OWNER admin;`} />

    <DockerTerminalOneLiner pathInfo="admin=#"command={String.raw`-- Connect to database
    \c "ln-history-database"`} />

    <DockerTerminalOneLiner pathInfo="admin=#" command={String.raw`-- Create nodes table
    CREATE TABLE nodes (
    node_id VARCHAR(66) PRIMARY KEY,
    from_timestamp TIMESTAMPTZ,
    last_seen TIMESTAMPTZ);"`} />

    <DockerTerminalOneLiner pathInfo="admin=#"command={String.raw`-- Create nodes_raw_gossip table
    ...
    <DockerTerminalOneLiner pathInfo="admin=#" command={String.raw`-- Create channels table
    ...
    <DockerTerminalOneLiner pathInfo="admin=#" command={String.raw`-- Create channel_updates table
...
**🔁 Replace** `YOUR_PASSWORD` with your actual password for the _ln-history-database_
